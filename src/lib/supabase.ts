...
export function mapSupabaseToProfile(row: any): UserProfile {
  return {
    ...
    credits: typeof row.credits === 'number' ? row.credits : (row.credits !== undefined && row.credits !== null ? parseInt(row.credits, 10) : 100),
    xp: typeof row.xp === 'number' ? row.xp : (row.xp !== undefined && row.xp !== null ? parseInt(row.xp, 10) : 0),
    loginStreak: typeof row.login_streak === 'number' ? row.login_streak : (row.login_streak !== undefined && row.login_streak !== null ? parseInt(row.login_streak, 10) : 1),
    longestStreak: typeof row.longest_streak === 'number' ? row.longest_streak : (row.longest_streak !== undefined && row.longest_streak !== null ? parseInt(row.longest_streak, 10) : 1),
...
export function mapProfileToSupabase(profile: UserProfile): any {
  return {
    ...
    xp: profile.xp ?? 0,
    login_streak: profile.loginStreak ?? 1,
    longest_streak: profile.longestStreak ?? 1,
...
export function mapBookingToSupabase(booking: Booking): any {
  const row: any = {
    ...
    skill_level: booking.skillLevel || null,
...
export function mapSupabaseToReview(row: any): Review {
  return {
    ...
    revieweeId: row.reviewee_id || undefined,
    reviewerRole: row.reviewer_role || undefined,
    revieweeName: row.reviewee_name || undefined,
    skillName: row.skill_name || '',
...
export function mapReviewToSupabase(review: Review): any {
  const row: any = {
    ...
    reviewee_name: review.revieweeName || null,

...
export function SessionCompleteSummary({
  ...
  // Check if learner has already submitted a review for this booking
  const existingReview = allReviews.find(
    r => r.bookingId === booking.id || (r.learnerId === currentUser.id && r.teacherId === booking.teacherId && r.skillName === booking.skillName)
  );
  const [hasReviewed, setHasReviewed] = useState(!!existingReview);
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      ...
      >
        ...
        {/* Review Form / Status Section (Only for Learner) */}
        {isLearner && (
          <div className="space-y-3 relative z-10">
            {hasReviewed ? (
              <div className="bg-emerald-950/30 border border-emerald-500/30 rounded-2xl p-3.5 flex items-center justify-between text-xs text-emerald-300">
                ...
                <span className="text-emerald-400 font-medium">Thank you for rating {teacherName}</span>
              </div>
            ) : showReviewForm ? (
              <form onSubmit={handleReviewSubmit} className="bg-slate-800/90 border border-indigo-500/40 rounded-2xl p-4 space-y-3 animate-in fade-in duration-200">
                <div className="flex items-center justify-between">
                  <h4 className="font-bold text-xs text-white">Leave a Review for {teacherName}</h4>
                  <button
                    type="button"
                ...
                <textarea
                  ...
                  placeholder={`Write a brief review about your experience with ${teacherName}...`}
                  rows={2}
              ...
              >
                <Star className="w-4 h-4 text-amber-400 fill-amber-400" />
                <span>Leave a Review for {teacherName}</span>
              </button>
            )}
          </div>
        )}

...
export default function ExploreView({ currentUser, users, onBookSession, onOpenChat, isLoading }: ExploreViewProps) {
  ...
  const [resourceModalCategory, setResourceModalCategory] = useState<string | null>(null);
  ...
  return (
    <div id="explore-view-root" className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 space-y-4">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1.5 scrollbar-thin">
          {CATEGORIES.map(category => {
            ...
            const hasResources = category !== 'All' && !!FREE_CATEGORY_RESOURCES[category];
              ...
              <div key={category} className="flex items-center shrink-0">
                ...
                {hasResources && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setResourceModalCategory(category);
                    }}
                    title={`Free learning resources for ${category}`}
                    className="ml-1 p-1 text-emerald-700 hover:text-emerald-900 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200/80 rounded-full transition cursor-pointer flex items-center justify-center shrink-0"
                  >
        ...
        {/* Free Resources Banner for Active Category */}
        {selectedCategory !== 'All' && FREE_CATEGORY_RESOURCES[selectedCategory] && (
          <div className="bg-emerald-50/90 border border-emerald-200/90 rounded-xl p-3 flex items-center justify-between gap-3 text-xs text-emerald-900">
            <div className="flex items-center gap-2 min-w-0">
              <div className="p-1.5 bg-emerald-100 text-emerald-800 rounded-lg shrink-0">
                <BookOpen className="w-4 h-4" />
              </div>
              <span className="truncate">
                Free learning guides available for <strong>{selectedCategory}</strong>!
              </span>
            </div>

            <button
              onClick={() => setResourceModalCategory(selectedCategory)}
              className="inline-flex items-center gap-1.5 font-bold text-emerald-800 hover:text-emerald-950 bg-white border border-emerald-300 hover:bg-emerald-100 px-3 py-1.5 rounded-lg shadow-2xs transition text-xs cursor-pointer shrink-0"
            >
      ...
      {/* FREE LEARNING RESOURCES MODAL */}
      <AnimatePresence>
        {resourceModalCategory && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 10 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 10 }}
              className="bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden text-xs text-slate-700 my-auto"
            >
              {/* Modal Header */}
              <div className="p-4 sm:p-5 border-b border-slate-200 bg-slate-50 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-emerald-100 text-emerald-800 rounded-xl flex items-center justify-center text-lg font-bold shrink-0 border border-emerald-200 shadow-2xs">
                    📚
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-bold text-slate-900">Free Learning Resources</h3>
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-full text-[10px] font-bold">100% Free</span>
                    </div>
                    <p className="text-slate-500 text-xs mt-0.5">
                      Curated tutorials, official courses, and YouTube guides for <strong className="text-slate-800">{resourceModalCategory}</strong>
                    </p>
                  </div>
                </div>

                <button
                  onClick={() => setResourceModalCategory(null)}
              ...
              <div className="px-4 py-2 bg-slate-100/70 border-b border-slate-200 flex items-center gap-1.5 overflow-x-auto scrollbar-thin shrink-0">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider shrink-0 mr-1">Category:</span>
                {Object.keys(FREE_CATEGORY_RESOURCES).map(cat => (
                  <button
                    key={cat}
                    onClick={() => setResourceModalCategory(cat)}
                    className={`px-3 py-1 rounded-lg text-xs font-semibold whitespace-nowrap transition cursor-pointer ${
                      resourceModalCategory === cat
              ...
              <div className="p-4 sm:p-6 overflow-y-auto space-y-4 flex-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {(FREE_CATEGORY_RESOURCES[resourceModalCategory] || []).map((resource, idx) => (
                    <a
                      key={idx}
                      href={resource.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="bg-white hover:bg-slate-50/80 border border-slate-200 hover:border-emerald-300 rounded-xl p-4 shadow-2xs hover:shadow-sm transition flex flex-col justify-between group no-underline text-slate-700 space-y-3 cursor-pointer"
                    >
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-bold text-slate-900 text-sm group-hover:text-emerald-700 transition flex items-center gap-1.5">
                            {resource.name}
                          </span>
                          {resource.type && (
                            <span className="px-2 py-0.5 bg-slate-100 text-slate-600 font-medium rounded-md text-[10px] shrink-0 border border-slate-200">
                              {resource.type}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-emerald-600 font-bold text-xs group-hover:text-emerald-700">
                ...
                <div className="bg-amber-50/80 border border-amber-200/80 rounded-xl p-3.5 flex items-start gap-2.5 text-amber-900 text-xs mt-2">
                  <Sparkles className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                  <div>
                    <p className="font-bold">Combine Free Study with Skill Swaps!</p>
                    <p className="text-slate-600 text-[11px] mt-0.5">
                      Use these free resources to build foundational knowledge, then connect with a swapper on ExchangeYourSkill to practice live, ask questions, or work on real projects together.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 sm:p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between shrink-0">
                <span className="text-slate-400 text-[11px]">
                  {FREE_CATEGORY_RESOURCES[resourceModalCategory]?.length || 0} curated free resources available
                </span>
                <button
                  onClick={() => setResourceModalCategory(null)}
